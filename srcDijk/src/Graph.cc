...
//: avaB(40, vector<vector<bool> >(Smax, vector<bool>(F, false))) //size for 6-node topology 
:avaB(225, vector<vector<bool> >(Smax, vector<bool>(F, false))) //size for nsfnet topology
{
	cout << "avaB size initialized in graph.cc is " << avaB.size() << ", 2nd dimension " << avaB[0].size() << ", 3rd dimension is: " << avaB[0][0].size() << endl;
...
void Graph::_import_from_file( const string& input_file_name )
{
	...
	//3.2 In the following lines, two types of lines are given. 
	///   one type line start with 's', each line indicates that the nodes and corresponding resources on each node.
	///   Another type line stars with 'l', each line contains a directed edge in the graph:
	///   the id of starting point, the id of ending point, the weight (distance) of the edge. 
	///   These values are separated by 'tab'. 
	int start_vertex, end_vertex;
	double edge_weight;
	int vertex_id = 0;

//self modified read in graph from graph file
//now the graph file include edge, node and resources capacity on each node.
	string line;
	char index;
	BaseVertex node;
	...
	while(std::getline(ifs, line))
	{
		...
		switch(index)
		{
			case 's': {
				...
				node.setID(atoi(newtoken.at(0).c_str()));
				node.setCn(atoi(newtoken.at(1).c_str()));
				node.setSn(atoi(newtoken.at(2).c_str()));
				node.setPn(atoi(newtoken.at(3).c_str()));
				node.setCPUCost(atof(newtoken.at(4).c_str()));
				node.setStorageCost(atof(newtoken.at(5).c_str()));
				node.setPortCost(atof(newtoken.at(6).c_str()));
				node.setIPNode(atof(newtoken.at(7).c_str()));
				node.setEONNode(atof(newtoken.at(8).c_str()));
			...
			case 'l': {
				...
				start_vertex = atoi(newtoken.at(0).c_str());
				end_vertex = atoi(newtoken.at(1).c_str());
				BaseVertex* start_vertex_pt = get_vertex(start_vertex);
				BaseVertex* end_vertex_pt = get_vertex(end_vertex);

				///3.2.2 add the edge weight (distance)
				edge_weight = atof(newtoken.at(2).c_str());
				m_mpEdgeCodeWeight[get_edge_code(start_vertex_pt, end_vertex_pt)] = edge_weight;
				//initial avaB for true in related position
				for(int i = 0; i < Smax; i++) {
					for(int j = 0; j < F; j++) {
						avaB[get_edge_code(start_vertex_pt, end_vertex_pt)][i][j] = true;
						//cout << get_edge_code(start_vertex_pt, end_vertex_pt) << " " << i << " " << j << endl;
						//cout << avaB.size() << endl;
						//cout << avaB[get_edge_code(start_vertex_pt, end_vertex_pt)][i][j] << "****" <<endl;
					}
				}
