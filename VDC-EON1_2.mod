 ...
 * OPL 12.6.0.0 Model
 * Author: panyi
 * Creation Date: Dec 29, 2014 at 2:54:36 PM
 * same problem model with VDC-EON1_1.mod. But Try different formular description, since 1_1 has running problem
 * 
 *********************************************/
 //with the following constraints, oplrun can only reach gap to 46.96%, best bound is 3601.3303 for 2 demands
tuple DC{
   key int v_ID;
   int C;
   int S;
...
// dvar boolean x_t[d in D][k in VV][v in V][t in 1 .. 24]; //VDC k mapped to DC v in time slot t
 ...
 forall(d in D)//, t in 1 .. 24: t == d.ST)
   capex[d] == sum(k in VV, v in V)(v.pn_i + v.pn_e)*x[d][k][v]*k.degree
   		+ sum(f in 1 .. F, l in VE, e in E) y[d][l][e][f] * ceil(l.RB/M/FSU)* ( e.dist * (C_FO+C_OA) + Reg10* floor(e.dist/reach) )
  ...
  forall(d in D, l in VE: l.d_ID==d.d_ID ) {
      forall(v1, v2 in V, k1, k2 in VV: k1.d_ID == d.d_ID && k2.d_ID == d.d_ID && l.vsrc==k1.vdc_ID && l.vdest==k2.vdc_ID  ) {
       ...
       forall(v in (V diff {v1, v2}))
         (x[d][k1][v1]==1 && x[d][k2][v2]==1) => sum(<i,v.v_ID,c> in E, f in 1 .. F) y[d][l][<i,v.v_ID,c> ][f] == sum(<v.v_ID,o,c> in E, f in 1 .. F) y[d][l][<v.v_ID,o,c>][f];
      }
  }  
   ...
   forall(d in D, l in VE, e in E, f in 1 .. F-1, t in d.ST .. d.ET: l.d_ID==d.d_ID) 
    ( y_t[d][l][e][f][t] - y_t[d][l][e][f+1][t] -1)* (-maxint) >= sum(f1 in f+2 .. F) y_t[d][l][e][f1][t];
     ...
     //the size of consecutive frequency slots is ceil(l.RB/M/FSU)
     //after adding the following two lines, gap 90.38% not convergence
   //forall(d in D, l in VE, e in E, f in 1 .. F, t in d.ST .. d.ET: l.d_ID==d.d_ID)
     //(y_t[d][l][e][f][t]-1)*maxint + ceil(l.RB/M/FSU) <= sum(f1 in 1 .. F) y_t[d][l][e][f1][t];
    
   forall(d in D, l in VE, e in E, f in 1 .. F, t in d.ST .. d.ET-1)
     y_t[d][l][e][f][t] == y_t[d][l][e][f][t+1];
